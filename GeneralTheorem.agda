...
general-imperfect-theorem : {st st₁ st₂ : ST}
                              → ∀ (m : ST-Moduli st₂)
                              ...
                              → let
                                  ...
                                  → ST-≈ᶠ st₁ st₂ Ω ω m
general-imperfect-theorem {st} {st₁} {st₂} m = reg , γ where
  X = ST-Type st
...
general-imperfect-corollary : {st st₁ st₂ : ST}
                              → ∀ (m : ST-Moduli 𝕌*)
                              → (Φ : Y st₁ st₂ → Y st₁ st₂ → 𝕌)
                              ...
                              → let
                                  ...
                                  → ST-≈ 𝕌* (Φ Ω ΨΩ) (Φ Ω ω) m
general-imperfect-corollary {st} {st₁} {st₂} m Φ cᴸ = reg , γ where
  reg = pr₁ (general-imperfect-theorem (pr₁ (cᴸ m)))
  γ : (M : ST-Type st → Y st₁ st₂)
        (cᴹ : continuous² (×* st st₁) st₂ (uncurry M))
        (Ψ : Y st₁ st₂ → Y st₁ st₂) (cᴰ : continuousᴰ st₁ st₂ Ψ)
        (k : ST-Type st) →
  ...
  γ M cᴹ Ψ cᴰ k Ω≈ΨΩ = pr₂ (cᴸ m) ΨΩ ω step₃ Ω where
    ...
    step₂ = pr₂ (general-imperfect-theorem (pr₁ (cᴸ m))) M cᴹ Ψ cᴰ k Ω≈ΨΩ
    step₃ : ST-≈ᶠ st₁ st₂ ΨΩ ω (pr₁ (cᴸ m))
